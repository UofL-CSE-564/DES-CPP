...
std::uint64_t bytesToBlock(const std::array<unsigned char, 8>& bytes, std::streamsize count) {
    std::uint64_t block = 0;
    for (std::size_t i = 0; i < bytes.size(); ++i) {
        block <<= 8;
        if (static_cast<std::streamsize>(i) < count) {
            block |= static_cast<std::uint64_t>(bytes[i]);
        }
    }
    return block;
}
...
DES::DES()
    : key_(0), has_key_(false) {}

DES::DES(std::uint64_t key)
    : key_(0), has_key_(false) {
    setKey(key);
}

void DES::setKey(std::uint64_t key) {
    if (!hasValidParity(key)) {
        throw std::invalid_argument("DES keys must have odd parity in each byte");
    }
...
bool DES::hasValidParity(std::uint64_t key) noexcept {
    for (int byte_index = 0; byte_index < 8; ++byte_index) {
        const auto byte = static_cast<unsigned char>((key >> (byte_index * 8)) & 0xffU);
        unsigned int ones = 0;
...
bool DES::hasKey() const noexcept {
    return has_key_;
}

std::uint64_t DES::key() const {
    requireKey();
    return key_;
}
...
std::uint64_t DES::encrypt(std::uint64_t block) const {
    requireKey();
    return des(block, key_, 'e');
}

std::uint64_t DES::decrypt(std::uint64_t block) const {
    requireKey();
    return des(block, key_, 'd');
}

std::vector<std::uint64_t> DES::encrypt(std::istream& input) const {
    return processStream(input, 'e');
}

std::vector<std::uint64_t> DES::decrypt(std::istream& input) const {
    return processStream(input, 'd');
}

void DES::requireKey() const {
    if (!has_key_) {
        throw std::logic_error("DES key has not been set");
    }
}

std::vector<std::uint64_t> DES::processStream(std::istream& input, char mode) const {
    requireKey();
    ...
    while (true) {
        ...
        const std::uint64_t block = bytesToBlock(bytes, count);
        output.push_back(mode == 'd' ? decrypt(block) : encrypt(block));
    ...
    if (input.bad()) {
        throw std::ios_base::failure("Failed while reading DES input stream");
    }
